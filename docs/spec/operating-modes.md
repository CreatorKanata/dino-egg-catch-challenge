...
The pick policy is trained in the LeRobot fork (ACT first, SmolVLA as a comparison) and consumed here through a policy runner that can be interrupted. Demonstrations are recorded with this repository's recorder (`robot.recording.record_pick_egg`), which drives the fork's `record_loop` with the leader arm and keyboard teleoperators (owner decision, 2026-09-25; the `lerobot-record` CLI cannot drive LeKiwi with a lone arm teleoperator): the robot is parked on a floor mark, eggs are placed at a marked best position with small deliberate offsets inside the alignment tolerance, and each episode runs from the catch pose (head down, egg in the wrist view) until the egg is held. The application reproduces that start state at demo time through alignment and the catch pose.

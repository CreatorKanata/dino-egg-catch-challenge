...
1. **Masks** (`vision/egg_masks.py`): the white body (`EGG_BODY_HSV`) and one mask per spot color (`EGG_SPOT_HSV`), all AND NOT the pink basket (`BASKET_HSV`), so the basket never joins an egg or adds spots. Red spots need S >= 150 to stay apart from the pink.
2. **Spots:** with `EGG_SPOT_DETECTOR = "hsv"` (default) these are connected components of the spot masks of at least `EGG_MIN_SPOT_AREA_PX`. With `"edge"`, they are OpenCV contrib EdgeDrawing ellipses whose inside is at least `EGG_EDGE_SPOT_MIN_FILL` of one spot color (`vision/spot_edges.py`); without `cv2.ximgproc` it falls back to `"hsv"`. On the check captures `"hsv"` found one egg in all seven test images at 8-11 ms. `"edge"` found fewer spots, matched on the five robot captures, but split the old channel-swapped screenshot into two half eggs, at 10-13 ms.
3. **Clusters:** spots link when their centers are closer than `EGG_SPOT_CLUSTER_FACTOR` (3.0) × the larger spot diameter (single linkage). A cluster's scale `d` is the median diameter of its core spots, those at least `EGG_CORE_SPOT_FACTOR` (0.5) × its largest spot; small greenish blobs from dark tarp folds may join but do not shrink the scale. Clusters with `d` below `EGG_MIN_SPOT_DIAMETER_PX` (10 px, an egg under ~40 px) are skipped.
...
| `20260924-220336` | pink basket behind | cx 0.390, cy 0.470, w 0.439, h 0.427 |
| `20260924-220404` | no basket | cx 0.395, cy 0.469, w 0.448, h 0.429 |
| `20260924-222325` | far egg | cx 0.512, cy 0.427, w 0.319, h 0.321 |

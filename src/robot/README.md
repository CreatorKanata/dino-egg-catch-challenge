...
| `vision/spot_edges.py` | Optional EdgeDrawing spot stage (`EGG_SPOT_DETECTOR = "edge"`), color-verified; falls back to HSV without `cv2.ximgproc` | None (OpenCV contrib) |
...
**Detection.** The detector runs every frame in Manual Mode on the front frame, in the parent process. It is anchored on the spots and scaled by their size. Under strong light, glints on the tarp's wrinkles are as white as the egg and cannot be separated by color, only by size (robot run 2026-09-24, capture 20260924-223853). All tunables are in `vision/config_vision.py`.

1. **Masks** (`vision/egg_masks.py`): the white body (`EGG_BODY_HSV`) and one mask per spot color (`EGG_SPOT_HSV`), with the pink basket (`BASKET_HSV`) removed.
   - Red spots are H 0-10 or 174-179 with S >= 140 and V >= 25. The shadowed centre spot of the red egg reaches V 33 (captures 225648/225701).
   - The basket is two ranges that do not overlap the red spots: H 150-173 at S >= 60, plus H 174-179 only below S 140 (basket in 220336: H 135-176, S 77-182).
   - The body and the green and blue masks drop the whole basket; the red mask drops only the first range. A few basket pixels at H 174-176 with S above 140 can still look red.
   - Spots link only with spots of the same color, and each cluster's component uses only its own color. So red-looking basket pixels never join a green egg, and on their own they fail the shape rules.
2. **Spots:** with `EGG_SPOT_DETECTOR = "hsv"` (default) these are connected components of the spot masks of at least `EGG_MIN_SPOT_AREA_PX`. With `"edge"`, they are OpenCV contrib EdgeDrawing ellipses whose inside is at least `EGG_EDGE_SPOT_MIN_FILL` of one spot color (`vision/spot_edges.py`); without `cv2.ximgproc` it falls back to `"hsv"`. On the check captures `"hsv"` found one egg in all ten test images at 9-12 ms. `"edge"` found fewer spots, matched on the eight robot captures, but split the old channel-swapped screenshot into two half eggs, at 10-13 ms.
3. **Clusters:** spots of the same color link when their centers are closer than `EGG_SPOT_CLUSTER_FACTOR` (3.0) × the larger spot diameter (single linkage; an egg has one spot color). A cluster's scale `d` is the median diameter of its core spots, those at least `EGG_CORE_SPOT_FACTOR` (0.35) × its largest spot. Small greenish blobs from dark tarp folds may join but do not shrink the scale, and the red egg's one oversized centre spot (d 140 against 34-79) does not inflate it. Clusters with `d` below `EGG_MIN_SPOT_DIAMETER_PX` (10 px, an egg under ~40 px) are skipped.
4. **Segmentation per cluster** (`vision/egg_detector.py`): inside a window around the core spots grown by `EGG_WINDOW_FACTOR` (2.0) × `d` per side, the basket-free body+spot mask is closed (`EGG_CLOSE_KERNEL_PX`, bridging the shell's crack) and opened with `EGG_OPEN_SPOT_FACTOR` (0.5) × `d`, which removes glints thinner than half a spot. The component holding the core spots is kept. Morphology runs on a downscaled copy so kernels stay near 15 px.
5. **Rules:** at least `EGG_MIN_AREA_PX`, bbox aspect in `EGG_ASPECT_RANGE`, and scale (bbox height / `d`) in `EGG_SCALE_RANGE` (2.0-6.5; 3.25-4.1 on all eight check captures). The outline is then gap-repaired (a close of `EGG_REPAIR_KERNEL_FRACTION` × the bbox's shorter side) and must have solidity >= `EGG_MIN_SOLIDITY` (0.85) and, away from the frame border (`EGG_BORDER_MARGIN_PX`), contour area / fitted-ellipse area in `EGG_ELLIPSE_FILL_RANGE` (0.75-1.25). Spot pixels inside must be at least `EGG_MIN_SPOTS` blobs and `EGG_MIN_SPOT_FRACTION` (0.12) of the egg area (0.25-0.37 on real eggs). A partly visible egg cut by the border keeps only the solidity test and is reported with `touches_border`.
6. **Result:** the color with the most spot pixels names the egg; overlapping results from two clusters are merged; the largest egg is used.

Each detection carries its bbox, which alignment and the size precondition use, and the ellipse fitted to the repaired outline (`EggDetection.ellipse`: center, axes, angle), which the signboard and the inspect tool draw. `BASKET_HSV` will be reused by the Auto Release basket detector. The detector needs about 9-11 ms per frame on the captures. The signboard's front view always shows the target as an egg-shaped outline labelled "place the egg here" just below it (also a guide for attendees driving by hand), and the best egg's fitted (possibly rotated) ellipse as a green outline ("green egg", label above it) when its size is usable, or as an accent-colored outline ("green egg: too far") otherwise. The detector's average time over the first 30 frames is logged once at INFO.
...
**Values are placeholders to calibrate at the venue.** The best position (`ALIGN_TARGET_CX` 0.49, `ALIGN_TARGET_CY` 0.53, `ALIGN_TARGET_W` 0.62, `ALIGN_TARGET_H` 0.60) is the detector's own measurement of capture `20260924-220742`, taken by the owner with the egg at the best position, so target and detector agree. The detector values were checked against the captures `20260924-220742`, `20260924-220336`, `20260924-220404`, `20260924-222325`, `20260924-223853`, and the red-spotted `20260924-225648`, `20260924-225701`, and `20260924-230143`. The tolerances and speeds are proposals. Recalibrate at the venue:
...
- **Phase 2 step 1 (unit tests; the owner's first robot run on 2026-09-24 found the basket false detection and the oscillation, both addressed here but not yet re-run on the robot):** the size precondition and its alerts, the alignment controller (signs, taper, tolerance zero, deadband, smoothing, rate limit, done / lost / timeout, trace rows), the loop wiring with a patched detector (controller and leader ignored, input loss not stopping it, Stop cancelling it, capture), the RGB -> BGR conversion, the overlays through the protocol and headless rendering, the capture key, and the detector on synthetic frames. Offline checks on real front captures from the robot (not unit-test fixtures), each one green egg and nothing else:

| Capture | Scene | Detected egg |
| --- | --- | --- |
| `20260924-220742` | best position | cx 0.489, cy 0.525, w 0.619, h 0.600 |
| `20260924-220336` | pink basket behind | cx 0.387, cy 0.471, w 0.445, h 0.425 |
| `20260924-220404` | no basket | cx 0.395, cy 0.469, w 0.448, h 0.429 |
| `20260924-222325` | far egg | cx 0.512, cy 0.429, w 0.322, h 0.308 |
| `20260924-223853` | strong light, tarp glints | cx 0.502, cy 0.525, w 0.527, h 0.575 |
| `20260924-225648` | red-spotted egg, best position | red, cx 0.508, cy 0.509, w 0.647, h 0.569 |
| `20260924-225701` | red-spotted egg, extra light | red, cx 0.509, cy 0.515, w 0.644, h 0.579 |
| `20260924-230143` | red-spotted egg, pink basket directly behind | red, cx 0.510, cy 0.546, w 0.642, h 0.654 |

...
| `mode_manager.py` | Frozen `AppState` (mode, action, voice input, notice, Stop flag) and the command rules | None (stdlib only) |
...
2. On the Mac, from this repository root, use the conda env `lerobot312` (owner decision): `conda activate lerobot312`, or call `/opt/miniconda3/envs/lerobot312/bin/python` directly. The env has the LeRobot fork installed editable (`pip install -e ".[lekiwi,viz]"` in `../lerobot-dino-egg-catch-challenge`) plus pyserial and pygame-ce. Check that it imports the fork; the path must be under `lerobot-dino-egg-catch-challenge`:
   ...
   The root `pyproject.toml` describes the same dependencies for an alternative `uv` setup. The leader arm calibration `dino_leader_arm.json` must be in `~/.cache/huggingface/lerobot/calibration/teleoperators/so_leader/` (see the guide, section 3). Then start Manual Mode:
...
| --- | --- | --- | --- |
| `Go Go!` | `mode_toggle` | Switch to FSC | Switch to Manual Mode |
...
**`Stop`** switches to Manual Mode, cancels any action, clears voice input, sends zero base velocities in that frame, discards pending encoder rotation, disengages arm following, and holds the arm at its last commanded pose with torque kept on (the host only releases torque when it disconnects). The signboard shows `STOP`. `Stop` does not close the application. The base follows the controller again from the next frame. The arm stays held while the Stop flag is set, which is until the next `Go Go!`, `Hi!`, or `Thx` press; after that it resumes through the slow engagement. ESC, the window close button, and Ctrl+C still quit the application through the zeros-first shutdown.
...
- **Leader ignored:** in FSC, while the Stop flag is set, or with `--no-leader`, the arm holds its pose and the leader is not read.

## Display

The attendee display is a pygame window (owner decision, 2026-09-24): the overhead camera large on the left, the front and wrist cameras stacked on the right, and a three-line status bar. Line 1 (large) is the mode: `MANUAL` or `FSC (demo)`, followed by the running action when there is one. Line 2 is the current notice if present, otherwise the drive status (`DRIVE`, `CATCH!`, `INPUT LOST`). Line 3 is the arm status (`arm holding`, `arm syncing`, `arm following`, `LEADER ARM FAULT`, `no leader arm`), `mic on` while FSC voice input is toggled on, the speed level as footprints (fixed by config), and the held directions plus a rotation symbol (↺/↻, ASCII `CCW`/`CW`) while an encoder rotation is pending. The bar turns the warning color on input loss and while the `STOP` notice is shown. A missing frame shows "no signal". Unicode symbols are used only when pygame's default font has them; otherwise ASCII fallbacks (`#`/`-`, `UP`/`RIGHT`) are shown.
...
| Shaft button | Catch request: the base stops while it is held; no arm motion. The spec gives the button no role for now (Catch moved to `Hi!`); the old default is kept until it is removed | Default kept, see the spec |
...
- **Stops clear pending rotation.** Input loss, Catch, `Stop`, every mode switch, and FSC all reset the encoder rotation budget to zero, so rotation never resumes on its own after a stop.
...
- **`Stop` and mode switches -> zeros, arm held.** See "KachiButton controls". This is a software stop through the signboard process and the loop (tens of milliseconds); the host watchdog remains the last line.
...
- **Verified without hardware:** the unit tests above pass. Base driving: controller framing, parsing, synchronization, action mapping, the encoder rotation budget, Catch and input-lost stops, arm-pose capture and nine-key action content with a fake client. Phase 1 additions (unit tests only): KachiButton phrase matching, the mode manager rules, slow engagement and following, the leader-arm adapter with a fake teleoperator, the adapter updating its held pose, the per-frame composition and loop wiring with fake devices (Stop, mode switch, leader following, leader fault), the protocol status fields and command lines, headless pygame rendering and `TEXTINPUT` collection with `SDL_VIDEODRIVER=dummy`, a fake child sending command lines to `SignboardClient`, a headless round trip through a real signboard child, and the import isolation of both processes. The hardware modules import and `--help` runs in the `lerobot312` environment.
- **Owner-tested on the robot (2026-09-24):** the base-driving half only: the owner drove the LeKiwi base with the dino-controller through `robot.teleop_drive` (then Drive Mode) and reported that the controller operation works. Not tested on the robot or with real devices: the leader arm and slow engagement, KachiButton input into a real signboard window, `Stop`, mode switching, the measured rotation per encoder click, the overhead camera, and the signboard in a real (non-headless) window. Each further physical run still needs the scope, reachable stop control, operating area, and verified limits agreed with the operator (see `AGENTS.md`).
- **Owner decisions (2026-09-24):** Manual Mode, the KachiButton mapping and signboard focus, `Stop` as a full stop with the arm held, slow engagement, joystick translation, encoder rotation, fixed speed level. One click turns the base by half the knob's own angle per click (`ENCODER_ROTATION_SCALE = 0.5`); `ENCODER_CLICKS_PER_REVOLUTION` (20) is an unmeasured placeholder, and `ENCODER_MAX_PENDING_DEG` (360°) is a runaway guard to tune on the robot.
- **Proposed defaults:** `ARM_ENGAGE_SPEED_DEG_S` (30) and `ARM_ENGAGE_TOLERANCE_DEG` (3), `NOTICE_SECONDS` (3), the shaft-button Catch default, and the controller timeouts.

...
| `align.py` | Auto Catch base alignment: proportional control of the egg's image position and size toward the best position, done / lost / timeout | None (stdlib only) |
...
| `vision/egg_detector.py` | HSV color detector: white body plus green/blue/red spots -> `detect_eggs`, `best_egg` | None (numpy, OpenCV loaded lazily) |
...
There is no dialog to dismiss (attendees have no cursor): alerts are timed notices only. During the alignment the controller's base input and the leader arm are ignored (the arm holds its last pose, status `arm holding`), and `Hi!`, `Thx`, and `Go Go!` are ignored. A proportional controller (`align.py`) drives the base toward the **best position**, the egg pose in the front image that the pick policy will start from: sideways from the horizontal error (egg right of the target -> move right, `-y`; gain `ALIGN_GAIN_Y` 0.8) and forward/backward from the bbox-height error (egg smaller than the target -> forward, `+x`; gain `ALIGN_GAIN_X` 0.5), no rotation. An axis already inside its tolerance gets 0, commands are clamped to `ALIGN_MAX_XY` (0.1 m/s, the slow level), and anything below `ALIGN_MIN_XY` (0.03 m/s) becomes 0 so the base does not creep. `config.py` refuses gains for which the command just outside a tolerance would fall below `ALIGN_MIN_XY` (`ALIGN_GAIN_Y * ALIGN_TOL_CX` and `ALIGN_GAIN_X * ALIGN_TOL_H` must be at least `ALIGN_MIN_XY`); otherwise the base could stop short of "done" and wait for the timeout. The run ends with:

- **done** after `ALIGN_DONE_FRAMES` (10) consecutive frames within `ALIGN_TOL_CX` (0.04) and `ALIGN_TOL_H` (0.06): "Aligned. Catch: not available yet" (the catch is Phase 3);
- **lost** after `ALIGN_LOST_FRAMES` (15) consecutive frames without an egg (the base is at zero meanwhile): warning "Egg lost";
...
**All values are placeholders from one screenshot.** The HSV ranges, the best position (`ALIGN_TARGET_CX` 0.56, `ALIGN_TARGET_CY` 0.55, `ALIGN_TARGET_W` 0.66, `ALIGN_TARGET_H` 0.62: the detector's own measurement of that screenshot, so target and detector agree), the tolerances, and the gains come from one front-camera screenshot and must be recorded and calibrated at the venue:
...
- **Alignment is slow, cancellable, and bounded.** The Auto Catch alignment never exceeds `ALIGN_MAX_XY` (0.1 m/s, the slow level) on either axis and never rotates; `Stop` cancels it at once, and it ends by itself after `ALIGN_TIMEOUT_S` (15 s) or when the egg is lost. Loss of dino-controller input does not stop it (the attendee is not driving), but `Stop`, ESC/window close, Ctrl+C, and the host watchdog still apply.
...
- **Phase 2 step 1 (unit tests only, not run on the robot):** the size precondition and its alerts, the alignment controller (signs, clamping, deadband, done / lost / timeout), the loop wiring with a patched detector (controller and leader ignored, input loss not stopping it, Stop cancelling it, capture), the RGB -> BGR conversion, the overlays through the protocol and headless rendering, the capture key, and the detector on synthetic frames. On the channel-swapped screenshot the placeholders came from (read with `--rgb`), the detector finds one green egg at cx 0.562, cy 0.553, w 0.661, h 0.623; the dark bottom of the egg falls outside the body mask, so this is less than the egg's visible height. The `ALIGN_TARGET_*` placeholders are set to this measurement. The best position must be recorded with a capture and the thresholds calibrated at the venue before any alignment run on the robot.

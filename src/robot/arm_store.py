"""src/robot/arm_store.py: Staff keys that record the Auto Release data, and loading it for `Thx`.

Staff record the home pose and the release motion from the signboard keyboard in Manual Mode with
the leader arm following (src/robot/README.md, "Auto Release"). `b` (HOME_KEY) saves the current
commanded arm pose as the home pose. `r` (RECORD_KEY) starts recording the commanded pose every
...
saves itself after RECORD_MAX_S. At a `Thx` press the loop loads both files into a ReleaseRequest; a missing or invalid file becomes None, so
Auto Release refuses with a warning and nothing moves. Stdlib-only; the file writes are the only
side effects, and every failure is logged and shown, never raised.
"""
...
HOME_NOTE: Final = "Home (base) pose: neck folded, mouth down. Commanded arm pose saved with the HOME_KEY."
...
class ArmDataPaths:
    """Where the home pose and the release motion live (the tests pass temporary paths)."""
...
def _load(loader: Callable[[Path], object], path: Path, what: str) -> object | None:
    try:
        ...
        logger.warning("No %s at %s; Auto Release needs it recorded first", what, path)
...
def handle_staff_keys(
    ...
    """Apply this frame's save-home and record keys, then record this frame's commanded pose.

...
The signboard child process rebuilds Drive Mode state and BGR frames from these packets, so
encoding, decoding, and rejection of truncated or malformed data are verified in memory.
"""
...
from robot.signboard_protocol import CLOSE_MESSAGE, CloseRequest, DisplayPacket, encode, read_packet
...
def packet(frames=(("top", FRAME), ("front", None))):
    return DisplayPacket(drive=DRIVE, controller=CONTROLLER, frames=frames)

...
ROW = TraceRow(t=0.5, cx_raw=0.61, w_raw=0.5, h_raw=0.4, top_raw=0.25, cx_smooth=0.6, w_smooth=0.52, x_vel=0.01,
...
class AlignTraceTests(unittest.TestCase):
    ...
    def test_header_once_then_one_line_per_row(self):
        ...
        append_row(path, ROW)
        append_row(path, TraceRow(1.0, None, None, None, None, 0.6, 0.52, 0.0, 0.0, "lost"))
        with path.open(newline="") as handle:
            rows = list(csv.reader(handle))
        self.assertEqual(rows[0], list(TRACE_COLUMNS))
        self.assertEqual(rows[0], ["t", "cx_raw", "w_raw", "h_raw", "top_raw", "cx_smooth", "w_smooth", "x_vel",
                                   "y_vel", "result"])
        self.assertEqual(rows[1], ["0.5000", "0.6100", "0.5000", "0.4000", "0.2500", "0.6000", "0.5200", "0.0100",
                                   "-0.0200", "running"])
        self.assertEqual(rows[2][:5] + rows[2][-1:], ["1.0000", "", "", "", "", "lost"])
